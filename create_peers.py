...
def create_stag_peers(HostList, edge_prob, pod_prob, flowsPerHost=1):
	"""
	...
	for host in HostList:
		...
		new_peers = []
		while len(new_peers) < flowsPerHost:
...
def create_random_peers(HostList, flowsPerHost=1):
	"""
		Create random iperf peers to generate traffic.
	"""
	peers = []
	for host in HostList:
		for i in xrange(flowsPerHost):
			peer = random.choice(HostList)
...
def create_peers():
	...
	if args.traffic.startswith('stag'):
		...
		flows_peers = create_stag_peers(HostList, edge_prob, pod_prob, 1)
	elif args.traffic.startswith('random'):
		flows_peers = create_random_peers(HostList, 1)
	else:
		pass
	...
	flows_num_per_host = float(args.flows_num_per_host)
	_flows_peers = flows_peers[0:int(host_num * flows_num_per_host)]

	# Write flows_peers into a file for reuse.
	file_save = open('iperf_peers.py', 'w')
	file_save.write('iperf_peers=%s' % _flows_peers)
	file_save.close()
